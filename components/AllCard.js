  ...
  const renderItem = ({item, index}) => {
    ...
    function CategoryList({ categoryName }) {
      const filteredData = products.filter((item) => item.item_cat1 == categoryName);
      return (
        <View style={styles.container}>
          ...
          />
          <Button title="Next" onPress={() => navigation.navigate('SelectService')} />
        </View>
      );
    }  
